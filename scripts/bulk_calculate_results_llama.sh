        ...
        for dataset in nq tqa; do 
            for split in "test" ; do
                for poison_type in "article" ; do 
                    mkdir -p ColBERT/aa/results_${dataset}_${split}_${llm}/${model}/$poison_type
                    for percent in 1 2 3 5 10 20 30 40 50 100 ; do 
                        mkdir -p results_${dataset}_${split}_${llm}/${model}/$poison_type/$percent/
